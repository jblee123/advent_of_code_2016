...
fn run_bot_rule(bots: &mut HashMap<u32, Bot>, chip_dest: &ChipDest,
    chip_num: u32, outputs: &mut HashMap<u32, u32>) {
    ...
    match chip_dest {
        ChipDest::Bot(targe_bot_num) => {
            ...
            run_bot(bots, *targe_bot_num, outputs);
        },
        ChipDest::Output(output_num) => {
            outputs.insert(*output_num, chip_num);
        },
    };
}
...
fn run_bot(bots: &mut HashMap<u32, Bot>, bot_num: u32,
    ...
    run_bot_rule(bots, &rules.lo_dest, lo_chip, outputs);
    run_bot_rule(bots, &rules.hi_dest, hi_chip, outputs);
}
...
fn run_bots(bots: &mut HashMap<u32, Bot>, inputs: &Vec<BotInput>,
    ...
    for input in inputs {
        ...
        run_bot(bots, input.bot_num, outputs);
    }
...
fn main() {
    ...
    match run_bots(&mut bots, &inputs, &mut outputs) {
        Err(e) => panic!("Error: {}", e),
        _ => {},
    };
    ...
    println!("{} * {} * {} = {}",
        output0, output1, output2, output0 * output1 * output2);
}
