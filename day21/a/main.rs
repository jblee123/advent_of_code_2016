...
fn do_move(s: &str, instr: &MoveInstr) -> String {
    ...
    let mut vec: Vec<u8> = Vec::new();
    vec.extend(s_bytes);
    let removed = vec.remove(instr.pos1 as usize);
    vec.insert(instr.pos2 as usize, removed);
    String::from_utf8(vec).unwrap()
}
...
fn main() {
    ...
    const START_TEXT: &str = "abcdefgh";

    let result = input.lines().fold(START_TEXT.to_string(), |acc, line| {
